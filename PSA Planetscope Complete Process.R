...
library(terra);ibrary(tidyverse);library(magrittr)
library(sf)
...
pl.files.summary <- list.files(
  ...
  mutate(
    ...
    term.max.diff = abs(date.max - cc_termination_date), ##gap between termination and available imagery
...
for(i in 1:length(orders.4.files)){
  ...
  for (i in 1:length(seasons)){
    ...
    # files.raster = files.raster[c(1:20),]
    ...
    for(j in 1:length(files.raster$file)){
      ...
      tryCatch({
        ...
        df.na = values(udm.mask$confidence) %>% is.na() %>% 
  ...
  for(i in 1:length(extraction.files$file)){
    ...
    df.long = pivot_longer(df,cols=c(2:df.cols)) %>% 
      mutate(
        ...
        field.date = paste0(code.rep,'_',date,'_',bands),##used to calculate #/scenes per day by polygon feature
